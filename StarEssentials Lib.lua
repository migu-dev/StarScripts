...
pcall(function()
	userinfo = HttpService:JSONDecode(readfile("Starlibinfo.txt"));
end)
...
local function SaveInfo()
	...
	userinfo["user"] = user
	userinfo["tag"] = tag
	writefile("Starlibinfo.txt", HttpService:JSONEncode(userinfo));
end
...
Discord.Name = "StarEssentials"
Discord.Parent = game.CoreGui
Discord.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

function Starlib:Window(text)
...
hotkeyminiz = false
	...
	DiscordInfo.Text = "StarEssentials v1                 Stable Host         Roblox Lua Engine    "
	...
	function Starlib:Notification(titletext, desctext, btntext)
